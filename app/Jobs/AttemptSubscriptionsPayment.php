...
class AttemptSubscriptionsPayment implements ShouldQueue
{
    use Queueable;
        ...
        Subscription::wherePaymentAttemptable()
            ->chunk(100, function ($subscriptions) {
                foreach ($subscriptions as $subscription) {
                    // process the payment
                    $paymentSuccessful = true; // $this->processPayment($subscription);
                    ...
                    if ($paymentSuccessful) {
                        ...
                        if ($subscription->failed_attempt_count < config('subscription.grace_period.attempts', 5)) {
                            $subscription->update([
                                ...
                                'failed_attempt_count' => $subscription->failed_attempt_count + 1,
